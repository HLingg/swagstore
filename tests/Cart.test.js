...
describe('Cart.applyDiscount()', () => {
  test('applies SWAG20 code for 20% off subtotal', () => {
    const c = new Cart();
    c.add(P1, 1); // 29.99
    const isApplied = c.applyDiscount('SWAG20');
    
    expect(isApplied).toBe(true);
    expect(c.discount).toBeCloseTo(29.99 * 0.2, 2);
    // Total = subtotal(29.99) + tax(2.40) - discount(6.00) = 26.39
    expect(c.total).toBeCloseTo(29.99 * 1.08 - (29.99 * 0.2), 2);
  });

  test('rejects invalid discount code', () => {
    const c = new Cart();
    c.add(P1, 1);
    const isApplied = c.applyDiscount('INVALID_CODE');
    
    expect(isApplied).toBe(false);
    expect(c.discount).toBe(0);
    expect(c.discountCode).toBeNull();
  });

  test('saves discountCode in toJSON for session persistence', () => {
    const c1 = new Cart();
    c1.add(P1, 1);
    c1.applyDiscount('SWAG20');
    
    const json = c1.toJSON();
    expect(json).toHaveProperty('discountCode', 'SWAG20');
    
    const c2 = new Cart(json); // Khôi phục từ session
    expect(c2.discount).toBeCloseTo(c1.discount, 2);
  });
});